...
class EbayScraper:
    ...
    def parse_data(self):
        try:
            ...
            price_span = (
                price_tag.find("span", {"class": "ux-textspans"}) if price_tag else None
            )
            ...
            second_img_button = self.soup.select(
                "button.ux-image-grid-item.image-treatment.rounded-edges"
            )
            second_img_url = (
                second_img_button[1].find("img")["src"]
                if len(second_img_button) > 1
                else "N/A"
            )
            logging.debug(f"Parsed second_img_url: {second_img_url}")

            seller_tag = self.soup.find(
                "h2",
                {"class": "d-stores-info-categories__container__info__section__title"},
            )
            if seller_tag:
                seller_span = (
                    seller_tag.find(
            ...
            seller_info_tag = self.soup.find(
                "div", class_="d-stores-info-categories__container__details"
            )
            seller_joined_span = (
                seller_info_tag.find("span", {"class": "ux-textspans"})
                if seller_info_tag
                else None
            )
            seller_joined_at = (
                seller_joined_span.text.strip().replace("Joined ", "")
                if seller_joined_span
                else "N/A"
            )
            logging.debug(f"Parsed seller_joined_at: {seller_joined_at}")

            seller_items_sold_tag = self.soup.find_all(
                "div", class_="d-stores-info-categories__container__info__section__item"
            )
            ...
            seller_items_sold = (
                float(
                    seller_items_sold_span.text.replace("K", "000")
                    .replace("M", "000000")
                    .replace(".", "")
                    .strip()
                )
                if seller_items_sold_span
                else 0.0
            )
            logging.debug(f"Parsed seller_items_sold: {seller_items_sold}")

            shipping_tag = self.soup.find(
                "div", {"data-testid": "ux-layout-section__item"}
            )
            shipping_price_span = (
                shipping_tag.find(
                    "span",
                    {"class": "ux-textspans ux-textspans--BOLD ux-textspans--NEGATIVE"},
                )
                if shipping_tag
                else None
            )
            shipping_price = (
                shipping_price_span.text.strip() if shipping_price_span else "N/A"
            )
            ...
            for span in all_spans:
                ...
                if "days returns" in span.text:
                    try:
                        ...
                        logging.debug(
                            f"Failed to parse returns days from span: {span.text.strip()}"
                        )
                        returns_day = 0
